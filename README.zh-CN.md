...
一个用于浏览、管理和快速恢复 DeepSeek TUI 会话的本地桌面应用。
...
这个项目在此基础上提供了一个图形化界面，方便你搜索、分组、预览、收藏本地会话，打开对应 JSON 文件目录，并从界面中快速继续某个会话。
...
- 支持打开新的系统终端并执行 `deepseek-tui.cmd resume <session-id>`。

## 安全模型

`0.1.0` 版本默认把 DeepSeek TUI 的 session 文件当作只读数据。
...
应用不会上传会话内容，不会调用 AI 模型生成摘要，也不会修改 DeepSeek TUI 原始 JSON 文件。
